...
def test_nbdboot_plan_end_to_end(api: dict[str, object]) -> None:
    ...
    # Cmdline is ``pixie.*``-only: the ``bty.*`` backward-compat prefix
    # was removed. Each of nbd + image + overlay_size + server + mac is
    # spelled exactly once with ``pixie.`` and never with ``bty.``.
    assert "bty." not in plan
